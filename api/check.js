...
async function checkML(url) {
  ...
  const fetchUrl = idM ? `https://articulo.mercadolibre.com.ar/MLA-${idM[1]}` : url;
...
async function checkFacebook(url) {
  ...
  if (finalUrl && !finalUrl.includes('/marketplace/item/')) return { active: false, reason: 'redirected_away' };
  return { active: true, reason: 'active' };
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ active: true, reason: 'fetch_error' });
  }
};
